...
void FlightObserver::wait_for_launch()
{
  ...
  if (acceleration->z < this->_config->launch_acceleration)
  {
    return;
  }
...
void FlightObserver::wait_for_landing()
{
  ...
  Vec3f *acceleration = this->_imu->get_world_acceleration();
  // check if the z acceleration above landing acceleration
  if (acceleration->z > this->_config->landing_acceleration)
  {
    return;
  }
