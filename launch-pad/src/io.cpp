...
void IO::update()
{
    ...
    this->_voltage = raw_voltage * this->_config->voltage_factor + this->voltage_offset;

    // read and calculate the current pressure
    float raw_pressure = (float)analogRead(PRESSURE_PIN);
    this->_pressure = raw_pressure * this->_config->pressure_factor + this->pressure_offset;
